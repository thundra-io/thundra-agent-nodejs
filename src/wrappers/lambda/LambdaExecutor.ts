...
export function finishTrace(pluginContext: PluginContext, execContext: ExecutionContext, config: TraceConfig) {
    ...
    const { response, rootSpan, error, userError, triggerClassName, platformData, finishTimestamp } = execContext;
    const { originalEvent } = platformData;

    let resolvedError = error;
    // If
    // - there is user specified error
    // - and either the handler error is empty or generated by Thundra,
    // use user error
    if (userError && (!error || Utils.isThundraGeneratedError(error))) {
        resolvedError = userError;
    }

    if (resolvedError) {
        const parsedErr = Utils.parseError(resolvedError);
...
export function finishInvocation(pluginContext: PluginContext, execContext: ExecutionContext) {
    ...
    const { invocationData, error, userError } = execContext;
