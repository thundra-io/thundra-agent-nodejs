...
export class AWSIntegration implements Integration {
    ...
    static injectTraceLink(span: ThundraSpan, req: any, config: any): void {
        ...
        try {
            ...
            if (service) {
                traceLinks = service.injectTraceLink(span, req, config);
            }
        ...
        function wrapper(wrappedFunction: any, wrappedFunctionName: string) {
            ...
            return function AWSSDKWrapper(callback: any) {
                ...
                try {
                    ...
                    const service = AWSIntegration.serviceFactory(serviceName);
                    if (service) {
                        activeSpan = service.wrap(integration, request, config);
                    } else {
...
class AwsAthenaIntegration {
    ...
    static injectTraceLink(span: ThundraSpan, req: any, config: any): any {
...
export class AwsFirehoseIntegration {
    ...
    static injectTraceLink(span: ThundraSpan, req: any, config: any): any {
        ...
        if (operationName === 'putRecord') {
            ...
            if (data) {
                traceLinks = AwsFirehoseIntegration
                    .generateFirehoseTraceLinks(
                        region,
                        deliveryStreamName,
                        timestamp,
                        data);
    ...
    static wrap(integration: any,
        ...
        if (config.dynamoDBTraceInjectionEnabled) {
            if (operationName === 'putItem') {
                this.injectDynamoDBTraceLinkOnPut(request.params, activeSpan);
            } else if (operationName === 'updateItem') {
                this.injectDynamoDBTraceLinkOnUpdate(request.params, activeSpan);
            } else if (operationName === 'deleteItem') {
                this.injectDynamoDBTraceLinkOnDelete(request.params);
            }
        }
